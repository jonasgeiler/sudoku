...
import {globalStrategy} from './globalstrategy'
...
function createStrategy() {
	// 创建一个字典，键为字符串，值为数组
	const strategy = writable({"key":[]});

	userGrid.subscribe(($userGrid) => {
		let newHints = {"key":[]}
		//判断是否此策略下的此棋盘状态是否在字典中了
		if (!globalStrategy.hasUserGrid(userGrid.get(),strategyservice.getStrategylist())) {
			//若不在则调用 strategyService
			newHints = strategyservice.execute(userGrid.get());
			//将此策略下此棋盘状态的结果存进字典
			globalStrategy.addUserGrid(structuredClone(userGrid.get()),structuredClone(strategyservice.getStrategylist()), newHints);
		}
		else{
			newHints = globalStrategy.getStrategy(userGrid.get(),strategyservice.getStrategylist());
		}
		strategy.set(newHints); // 更新 strategy
		for (const item of newHints["SimpleStrategy"]) {
	...
	return {
		...
		// 用来直接更新整个字典
		set(newHints) {
			strategy.set(newHints);  // 直接用新的字典替换
		},
