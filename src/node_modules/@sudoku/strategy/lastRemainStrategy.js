...
export class LastRemianStrategy extends Strategy {
    ...
    findLastRemainInRow(allCandidates, row, allReason) {
        ...
        for (let candidate = 1; candidate <= SUDOKU_SIZE; candidate++){
            let position = []
            for (let col = 0; col < SUDOKU_SIZE; col ++) {
                if( allCandidates[row][col].length > 1  && allCandidates[row][col].includes(candidate)) {
                    position.push(col);
                } 
            ...
            if(position.length === 1) {
                ...
                allReason[row][position[0]] = ["Last Remaining in row."]
            }
        }
    }
    ...
    findLastRemainInCol(allCandidates, col, allReason) {
        ...
        for (let candidate = 1; candidate <= SUDOKU_SIZE; candidate++){
            ...
            if(position.length === 1) {
                ...
                allReason[position[0]][col] = ["Last Remaining in column."]
            }
        }
    }
    ...
    findLastRemainInBox(allCandidates, boxRow, boxCol, allReason) {
        ...
        for (let candidate = 1; candidate <= SUDOKU_SIZE; candidate++){
            ...
            if(position.length === 1) {
                ...
                allReason[position[0][0]][position[0][1]] =  ["Last Remaining in box."]
            }
        }
    }
}
