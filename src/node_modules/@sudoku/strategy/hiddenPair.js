        ...
        for (let num1 = 1; num1 <= SUDOKU_SIZE; num1++) {
            for (let num2 = num1 + 1; num2 <= SUDOKU_SIZE; num2++){
                ...
                if (combineSet.size === 2 && candidateCounts[num1].size >= 1 && candidateCounts[num2].size >= 1) {
                    ...
                    allReason[row][combineList[0]] = allReason[row][combineList[0]].concat([`[${row}, ${combineList[0]}] 和 [${row}, ${combineList[0]}] 是一对行Hidden Pair`]).filter(
                        element => !Array.isArray(element) || element.length !== 0);
                    allReason[row][combineList[0]] = [...new Set(allReason[row][combineList[0]])];
                    allReason[row][combineList[1]] = allReason[row][combineList[1]].concat([`[${row}, ${combineList[0]}] 和 [${row}, ${combineList[1]}] 是一对行Hidden Pair`]).filter(
                        element => !Array.isArray(element) || element.length !== 0);
                    allReason[row][combineList[1]] = [...new Set(allReason[row][combineList[1]])];
                    // 将同一行的无关元素全部清除
                    basicStragety.removeInRow(allCandidates, [[row, combineList[0]], [row, combineList[1]]], 
                        [num1, num2], row);
                    // 如果同时满足在同一个九宫格，同时消除九宫格里面的数字
                    if (basicStragety.isSameBox([[row, combineList[0]], [row, combineList[1]]])) {
                        basicStragety.removeInBox(allCandidates, [[row, combineList[0]], [row, combineList[1]]], 
                            [num1, num2], Math.floor(row / BOX_SIZE), Math.floor(combineList[0] / BOX_SIZE));
                    }
                }
            }
        }
        ...
        for (let num1 = 1; num1 <= SUDOKU_SIZE; num1++) {
            for (let num2 = num1 + 1; num2 <= SUDOKU_SIZE; num2++){
                ...
                if (combineSet.size === 2 && candidateCounts[num1].size >= 1 && candidateCounts[num2].size >= 1) {
                    ...
                    allReason[combineList[0]][col] = allReason[combineList[0]][col].concat([`[${combineList[0]}, ${col}] 和 [${combineList[1]}, ${col}] 是一对列Hidden Pair`]).filter(
                        element => !Array.isArray(element) || element.length !== 0);
                    allReason[combineList[0]][col] = [...new Set(allReason[combineList[0]][col])];
                    allReason[combineList[1]][col] = allReason[combineList[1]][col].concat([`[${combineList[0]}, ${col}] 和 [${combineList[1]}, ${col}] 是一对列Hidden Pair`]).filter(
                        element => !Array.isArray(element) || element.length !== 0);
                    allReason[combineList[1]][col] = [...new Set(allReason[combineList[1]][col])];
                    // 将同一列的无关元素全部清除
                    basicStragety.removeInCol(allCandidates, [[combineList[0], col], [combineList[1], col]], 
                        [num1, num2], col);
                    // 如果同时满足在同一个九宫格，同时消除九宫格里面的数字
                    if (basicStragety.isSameBox([[combineList[0], col], [combineList[1], col]])) {
                        basicStragety.removeInBox(allCandidates, [[combineList[0], col], [combineList[1], col]], 
                            [num1, num2], Math.floor(combineList[0] / BOX_SIZE), Math.floor(col / BOX_SIZE));
                    }
                }
            }
        }
        ...
        for (let num1 = 1; num1 <= SUDOKU_SIZE; num1++) {
            for (let num2 = num1 + 1; num2 <= SUDOKU_SIZE; num2++){
                ...
                if (combineSet.size === 2 && candidateCounts[num1].size >= 1 && candidateCounts[num2].size >= 1) {
                    ...
                    allReason[combineList[0][0]][combineList[0][1]] = allReason[combineList[0][0]][combineList[0][1]].concat([`[${combineList[0][0]}, ${combineList[0][1]}] 和 [${combineList[1][0]}, ${combineList[1][1]}] 是一对宫Hidden Pair`]).filter(
                        element => !Array.isArray(element) || element.length !== 0);
                    allReason[combineList[0][0]][combineList[0][1]] = [...new Set(allReason[combineList[0][0]][combineList[0][1]])];
                    allReason[combineList[1][0]][combineList[1][1]] = allReason[combineList[1][0]][combineList[1][1]].concat([`[${combineList[0][0]}, ${combineList[0][1]}] 和 [${combineList[1][0]}, ${combineList[1][1]}] 是一对宫Hidden Pair`]).filter(
                        element => !Array.isArray(element) || element.length !== 0);
                    allReason[combineList[1][0]][combineList[1][1]]= [...new Set(allReason[combineList[1][0]][combineList[1][1]])];
                    // 将同一九宫格的无关元素全部清除
                    basicStragety.removeInCol(allCandidates, [[combineList[0][0]][combineList[0][1]], [combineList[1][0]][combineList[1][1]]], 
                        [num1, num2], boxRow, boxCol);
                    // 如果同时满足在同一个行，同时消除这一行里面的数字
                    if (basicStragety.isSameRow([[combineList[0][0]][combineList[0][1]], [combineList[1][0]][combineList[1][1]]])) {
                        basicStragety.removeInRow(allCandidates, [[combineList[0][0]][combineList[0][1]], [combineList[1][0]][combineList[1][1]]], 
                            [num1, num2], combineList[0][0]);
                    }
                    // 如果同时满足在同一个列，同时消除这一列里面的数字
                    if (basicStragety.isSameCol([[combineList[0][0]][combineList[0][1]], [combineList[1][0]][combineList[1][1]]])) {
                        basicStragety.removeInRow(allCandidates, [[combineList[0][0]][combineList[0][1]], [combineList[1][0]][combineList[1][1]]], 
                            [num1, num2], combineList[0][1]);
                    }
