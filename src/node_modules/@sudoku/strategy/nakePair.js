                ...
                for (let nextCol = firstCol + 1; nextCol < SUDOKU_SIZE; nextCol ++) {
                    ...
                    if (allCandidates[row][nextCol].length === 2 && 
                        ...
                        allCandidates[row][firstCol][1] === allCandidates[row][nextCol][1]) {
                            allReason[row][firstCol] = allReason[row][firstCol].concat([`[${row}, ${firstCol}] 和 [${row}, ${nextCol}] 是一对行Naked Pair`]).filter(
                                element => !Array.isArray(element) || element.length !== 0);
                            allReason[row][firstCol] = [...new Set(allReason[row][firstCol])];
                            allReason[row][nextCol] = allReason[row][nextCol].concat([`[${row}, ${firstCol}] 和 [${row}, ${nextCol}] 是一对行Naked Pair`]).filter(
                                element => !Array.isArray(element) || element.length !== 0);
                            allReason[row][nextCol] = [...new Set(allReason[row][nextCol])];
                            basicStragety.removeInRow(allCandidates, [[row, firstCol], [row, nextCol]], 
                                [allCandidates[row][firstCol][0], allCandidates[row][firstCol][1]], row);
                    }
                }
            }
        }
                ...
                for (let nextRow = firstRow + 1; nextRow < SUDOKU_SIZE; nextRow ++) {
                    ...
                    if (allCandidates[nextRow][col].length === 2 && 
                        ...
                        allCandidates[firstRow][col][1] === allCandidates[nextRow][col][1]) {
                            // 显示理由
                            allReason[firstRow][col] = allReason[firstRow][col].concat([`[${firstRow}, ${col}] 和 [${nextRow}, ${col}] 是一对列Naked Pair`]).filter(
                                element => !Array.isArray(element) || element.length !== 0);
                            allReason[firstRow][col] = [...new Set(allReason[firstRow][col])];
                            allReason[nextRow][col] = allReason[nextRow][col].concat([`[${firstRow}, ${col}] 和 [${nextRow}, ${col}] 是一对列Naked Pair`]).filter(
                                element => !Array.isArray(element) || element.length !== 0);
                            allReason[nextRow][col] = [...new Set(allReason[nextRow][col])];
                            basicStragety.removeInCol(allCandidates, [[firstRow, col], [nextRow, col]], 
                                [allCandidates[firstRow][col][0], allCandidates[firstRow][col][1]], col);
                ...
                for (let nextBoxIndex = firstBoxIndex + 1; nextBoxIndex <SUDOKU_SIZE; nextBoxIndex ++) {
                    ...
                    if (allCandidates[nextRow][nextCol].length === 2 && 
                        ...
                        allCandidates[firstRow][firstCol][1] === allCandidates[nextRow][nextCol][1]) {
                            allReason[firstRow][firstCol] = allReason[firstRow][firstCol].concat([`[${firstRow}, ${firstCol}] 和 [${nextRow}, ${nextCol}] 是一对宫Naked Pair`]).filter(
                                element => !Array.isArray(element) || element.length !== 0);
                            allReason[firstRow][firstCol] = [...new Set(allReason[firstRow][firstCol])];
                            allReason[nextRow][nextCol] = allReason[nextRow][nextCol].concat([`[${firstRow}, ${firstCol}] 和 [${nextRow}, ${nextCol}] 是一对宫Naked Pair`]).filter(
                                element => !Array.isArray(element) || element.length !== 0);
                            allReason[nextRow][nextCol] = [...new Set(allReason[nextRow][nextCol])];
                            basicStragety.removeInBox(allCandidates, [[firstRow, firstCol], [nextRow, nextCol]], 
                                [allCandidates[firstRow][firstCol][0], allCandidates[firstRow][firstCol][1]], boxRow, boxCol);
                        }
                }
            }
        }
